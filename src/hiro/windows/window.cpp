...
auto pWindow::setIcon(const image& icon) -> void {
  HBITMAP hBitmap = CreateBitmap(icon);
  BITMAP bitmap;
  GetObject(hBitmap, sizeof(BITMAP), (LPSTR)&bitmap);

  HBITMAP mask = CreateCompatibleBitmap(GetDC(NULL), bitmap.bmWidth, bitmap.bmHeight);
  ...
  SendMessage(hwnd, WM_SETICON, ICON_BIG, (LPARAM)hIcon);  //task bar
  SendMessage(hwnd, WM_SETICON, ICON_SMALL, (LPARAM)hIcon);  //title bar
}

auto pWindow::setIcon(const image& iconBig, const image& iconSmall) -> void {
  for(uint n : range(2)) {
    HBITMAP hBitmap = CreateBitmap(n ? iconSmall : iconBig);
    BITMAP bitmap;
    GetObject(hBitmap, sizeof(BITMAP), (LPSTR)&bitmap);
    ...
    SendMessage(hwnd, WM_SETICON, n ? ICON_SMALL : ICON_BIG, (LPARAM)hIcon);
  }
}
