unique_pointer<VideoSettingsManager> videoSettingsManager;

VideoSettingsManager::VideoSettingsManager() {
  videoSettingsManager = this;
  ...
  horizontalMaskSlider.setLength(17).setPosition(settings["Video/Overscan/Horizontal"].natural()).onChange([&] { updateViewport(); });
  verticalMaskLabel.setText("Vertical:");
  verticalMaskValue.setAlignment(0.5);
  verticalMaskSlider.setLength(17).setPosition(settings["Video/Overscan/Vertical"].natural()).onChange([&] { updateViewport(); });
...
auto VideoSettingsManager::show() -> void {
  setVisible();
  setFocused();
  doSize();
}

auto VideoSettingsManager::updateColor(bool initializing) -> void {
...
auto VideoSettingsManager::updateViewport(bool initializing) -> void {
  ...
  if(!initializing) presentation->resizeViewport(true);
}
