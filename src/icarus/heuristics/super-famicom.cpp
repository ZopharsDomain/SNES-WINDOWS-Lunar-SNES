...
//private:
  ...
  enum HeaderField : uint {
    ...
    Company     = 0x2a,
    Version     = 0x2b,
    Complement  = 0x2c,  //inverse checksum
    Checksum    = 0x2e,
    ResetVector = 0x4c,
  };
...
SuperFamicomCartridge::SuperFamicomCartridge(const uint8_t* data, uint size, bool has_msu1) {
  ...
  else if(type == Type::GD3) {
    markup.append(
      "  rom name=program.rom size=0x", hex(rom_size), "\n"
      "    map address=00-7d,80-ff:8000-ffff base=0x000000 mask=0x8000\n"
      "    map address=40-6f:0000-7fff base=0x600000 mask=0x8000\n"
      "    map address=c0-ff:0000-7fff base=0x400000 mask=0x8000\n"
    );
    if(ram_size > 0) markup.append(
      "  ram name=save.ram size=0x", hex(ram_size), "\n"
      "    map address=00-3f,80-bf:6000-7fff mask=0xe000\n"
...
auto SuperFamicomCartridge::readHeader(const uint8_t* data, uint size) -> void {
  ...
  const uint8_t rom_size = data[index + RomSize];
        uint8_t company  = data[index + Company];
  ...
  const bool extended = company == 0x33;
  if(extended) company = slice((const char*)&data[index + ExCompany], 0, 2).hex();
  const uint8_t rom_sub_type = extended ? data[index + RomSubType] : 0x00;
  ...
  if(data[index - 14] == 'Z') {
    if(data[index - 11] == 'J') {
      ...
      if((n13 >= 'A' && n13 <= 'Z') || (n13 >= '0' && n13 <= '9')) {
        if(extended || (data[index - 10] == 0x00 && data[index - 4] == 0x00)) {
          has_bsx_slot = true;
        }
      }
    }
  ...
  if(has_bsx_slot) {
    if(!memcmp(data + index, "Satellaview BS-X     ", 21)) {
      ...
      type = (index == 0x7fb0 ? Type::LoROMSatellaview : Type::HiROMSatellaview);
      region = Region::NTSC;  //BS-X slotted cartridges only released in Japan
    }
  } else {
    //standard cart
    if(index == 0x7fb0 && size >= 0x401000) {
      type = Type::ExLoROM;
    } else if(index == 0x7fb0) {
      type = Type::LoROM;
    } else if(index == 0xffb0) {
      type = Type::HiROM;
    } else if(index == 0x40ffb0) {
      type = Type::ExHiROM;
    } else {
      type = Type::Unknown;
      return;
  ...
  if((rom_type & 0xf0) == 0x00 && (rom_type & 0x0f) >= 0x03) {
    has_dsp1 = true;

    if(mapperid == 0x20 && rom_type == 0x05 && company == 0x72) {
      has_dsp1 = false;
      has_dsp2 = true;
    }
    ...
    if(mapperid == 0x30 && rom_type == 0x03 && company == 0x7f) {
      has_dsp1 = false;
      has_dsp4 = true;
    }
  }

  if( mapperid == 0x20         && (rom_type & 0xf0) == 0x10 && (rom_type & 0x0f) >= 0x03) {
    has_superfx = true;
    ram_size = extended ? 1024 << (data[index + ExRamSize] & 7) : 0x8000;
    if(ram_size == 1024) ram_size = 0;
  }

  if((mapperid & 0xef) == 0x20 && (rom_type & 0xf0) == 0x20 && (rom_type & 0x0f) >= 0x03) {
    has_obc1 = true;
  }

  if( mapperid == 0x23         && (rom_type & 0xf0) == 0x30 && (rom_type & 0x0f) >= 0x02) {
    has_sa1 = true;
    type = Type::SA1;
  }

  if((mapperid & 0xef) == 0x22 && (rom_type & 0xf0) == 0x40 && (rom_type & 0x0f) >= 0x03) {
  ...
  if((mapperid & 0xef) == 0x25 && (rom_type & 0xf0) == 0x50 && (rom_type & 0x0f) >= 0x03) {
    has_sharprtc = true;
  }

  if((mapperid & 0xef) == 0x2a && (rom_type & 0xf0) == 0xf0 && rom_sub_type == 0x00) {
    has_spc7110 = true;
    type = Type::SPC7110;
    has_epsonrtc = (rom_type == 0xf9);
  }

  if((mapperid & 0xef) == 0x20 && (rom_type & 0xf0) == 0xf0 && rom_sub_type == 0x01) {
    if(rom_size >= 10) has_st010 = true;
    if(rom_size <  10) has_st011 = true;
  }

  if((mapperid & 0xef) == 0x20 && (rom_type & 0xf0) == 0xf0 && rom_sub_type == 0x02) {
    has_st018 = true;
  }

  if((mapperid & 0xef) == 0x20 && (rom_type & 0xf0) == 0xf0 && rom_sub_type == 0x10) {
    has_cx4 = true;
  }
}

auto SuperFamicomCartridge::findHeader(const uint8_t* data, uint size) -> uint {
  uint score_lo = scoreHeader(data, size, 0x007fb0);
  uint score_hi = scoreHeader(data, size, 0x00ffb0);
  uint score_ex = scoreHeader(data, size, 0x40ffb0);
...
auto SuperFamicomCartridge::scoreHeader(const uint8_t* data, uint size, uint addr) -> uint {
  if(size < addr + 80) return 0;  //image too small to contain header at this location?
  int score = 0;
  ...
  if(addr == 0x007fb0 && mapper == 0x20) score += 2;  //LoROM
  if(addr == 0x00ffb0 && mapper == 0x21) score += 2;  //HiROM
  if(addr == 0x007fb0 && mapper == 0x22) score += 2;  //S-DD1
  if(addr == 0x007fb0 && mapper == 0x23) score += 2;  //SA1
  if(addr == 0x40ffb0 && mapper == 0x25) score += 2;  //ExHiROM
  if(addr == 0x00ffb0 && mapper == 0x2a) score += 2;  //SPC7110
